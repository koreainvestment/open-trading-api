...
1. KIS Trade MCP 다운로드 및 인증정보 연동
    1. Docker 데스크탑 다운로드 및 설치
    ...
    6. **HTTP 서버 접근 확인 : 컨테이너가 정상적으로 실행되면 HTTP 서버에 접근이 가능합니다.**
...
2. Claude Desktop
Claude Desktop 설정 파일에 json 코드를 복사/붙여넣기해 KIS Trade MCP 서버 등록 후 저장하기
...
3. Cursor
Cursor를 실행하고 커맨트 팔레트에 OPEN MCP settings을 입력하면 MCP 설정탭이 오픈됩니다. 
    
    ![image5.png](./images/image5.png)
    
    `Add Custom MCP` 를 누르면, mcp.json 탭 오픈
    ...
    코드를 복사/붙여넣기 후 저장하면 설치 완료.
    
    ![image6.png](./images/image6.png)
    
    KIS Trade MCP가 연결되었는지 확인 (경로 :  `Settings` > `MCP Servers`)
...
*(Windows **PowerShell** 기준: `~`는 `C:\Users\내이름`으로 확장됩니다.)*
...
#실전투자
my_app: "발급받은_실전_APP_KEY"      # 한국투자증권에서 발급받은 APP KEY 입력
my_sec: "발급받은_실전_APP_SECRET"   # 한국투자증권에서 발급받은 APP SECRET 입력

#모의투자  
paper_app: "발급받은_실전_APP_KEY"      # 모의투자용 APP KEY (실전과 동일)
paper_sec: "발급받은_실전_APP_SECRET"   # 모의투자용 APP SECRET (실전과 동일)

# HTS ID
my_htsid: "실전_HTS_ID"              # 한국투자증권 HTS ID 입력

#계좌번호 및 8자리
my_acct_stock: "실전_계좌번호"        # 주식 계좌번호 (예: 50068418)  
my_acct_future: "실전_계좌번호"       # 선물옵션 계좌번호 (주식과 동일 가능)
