...
Postman 실행 후 아래 4개의 json 파일들을 Import 해주세요. Import하는 방법은 좌측 상단의 Import 버튼을 누르시고 File 탭에 파일을 끌어다 놓아주세요. 파일이 정상적으로 끌어와지면 아래 이미지와 같이 파일들이 식별되며, Import 버튼을 누르시면 됩니다.
