...
async def get_master_status() -> MasterStatus:
    ...
    kospi_mtime = _get_file_mtime(_get_csv_path("kospi"))
    kosdaq_mtime = _get_file_mtime(_get_csv_path("kosdaq"))

    return MasterStatus(
        kospi_count=len(kospi_symbols),
        kosdaq_count=len(kosdaq_symbols),
        total_count=len(kospi_symbols) + len(kosdaq_symbols),
...
async def get_symbol_detail(code: str) -> SymbolDetailResponse:
    """종목코드로 상세 정보 조회
    ...
    return SymbolDetailResponse(data=SymbolSearchItem(**stock))
