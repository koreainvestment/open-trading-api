...
class KISBrokerageProvider:
    ...
    def get_orders(self, status: Optional[OrderStatus] = None) -> List[Order]:
        ...
        Args:
            status: 필터링할 주문 상태. None이면 전체 조회.
        """
        ...
        if status == OrderStatus.FILLED:
            ccld_dvsn = "01"
        elif status in (OrderStatus.SUBMITTED, OrderStatus.PENDING):
            ccld_dvsn = "02"
        ...
        if not resp.is_ok():
            raise KISError(f"주문 내역 조회 실패: {resp.error_message}")
        ...
        for item in data:
            try:
                ...
                ord_price = float(item.get("ord_unpr", 0))
                ...
                logger.warning(f"주문 파싱 오류: {e}")
                continue

        logger.info(f"주문 내역 조회 완료: {len(orders)}건")
        return orders
    ...
    def subscribe_fills(
        ...
        if not hts_id:
            raise KISError(
                "HTS ID가 필요합니다. ~/KIS/config/kis_devlp.yaml의 my_htsid를 설정하세요."
            )
        ...
        return Subscription(id=sub_id, symbols=[], is_active=False)

    def subscribe_fills_async(
        self, on_fill: Callable[[Order], None], hts_id: Optional[str] = None
    ) -> "KISWebSocket":
        """체결 통보 구독 (비동기)
