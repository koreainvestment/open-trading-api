...
##############################################################################################
# [해외선물옵션] 주문/계좌 > 해외주식 전부취소주문[v1_해외주식-003]  (검증진행중)
##############################################################################################
# Input: None (Option) 상세 Input값 변경이 필요한 경우 API문서 참조
# Output: DataFrame (Option) output API 문서 참조 등
def get_overseasfuopt_order_allcncl(excg_cd="", itm_no="", tr_cont="", FK100="", NK100="", dataframe=None):
    url = '/uapi/overseas-futureoption/v1/trading/order-rvsecncl'
    ...
    if tr_cont == "D" or tr_cont == "E": # 마지막 페이지
        ...
        for index, row in current_data.iterrows():
            print(row['odno'])
            r_odno = row['odno']
            res_cncl = get_overseasfuopt_order_rvsecncl(excg_cd="NASD", itm_no="", orgn_odno=r_odno, rvse_cncl_dvsn_cd="02")
            print(res_cncl)
